...
function pureIntentAppData({ slippageBps, intent }) {
  const schema = intent?.cow_app_data_schema || {};
  const baseMetadata = schema.base_metadata || {};
  const orderClass = baseMetadata.orderClass?.orderClass || "limit";
  return {
    metadata: {
