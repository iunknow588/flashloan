...
def liquidation_account_execute_api():
    ...
    try:
        payload = liquidation_execution_payload_for_account(account, require_executor=False)
        return jsonify(execute_self_funded_liquidation_transaction(payload))
    except Exception as exc:
            ...
            response.update(
                {
                    ...
                    "account_report": account_report,
                    "execution_plan": account_report.get("execution_plan") if isinstance(account_report, dict) else None,
                    "execution_controls": payload.get("execution_controls") or liquidation_execution_controls(),
                }
            )
...
def liquidation_account_flashloan_api():
    ...
    try:
        payload = liquidation_execution_payload_for_account(account)
        return jsonify(execute_flashloan_liquidation_transaction(payload))
    except Exception as exc:
        response = {"error": str(exc), "account": account}
        if isinstance(payload, dict):
            account_report = payload.get("account_report") or {}
