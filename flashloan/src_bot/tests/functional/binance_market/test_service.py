...
def test_cow_quote_verification_quotes_selected_pairs(monkeypatch):
    ...
    monkeypatch.setattr("market.binance_market.service.build_token_registry", lambda *args, **kwargs: registry)
    ...
    monkeypatch.setattr("market.binance_market.service.evaluate_cow_route", fake_evaluate)
    monkeypatch.setattr(
        "market.binance_market.service.rank_cow_routes",
...
def test_cow_quote_verification_records_all_chain_candidates(monkeypatch):
    ...
    monkeypatch.setattr("market.binance_market.service.build_token_registry", lambda *args, **kwargs: registry)
    monkeypatch.setattr(
        "market.binance_market.service.evaluate_cow_route",
        lambda route, **kwargs: {
            "name": route["name"],
            "path": route["path"],
            "input_amount": "1000",
    ...
    monkeypatch.setattr("market.binance_market.service.rank_cow_routes", lambda items: items)
...
def test_cow_quote_verification_selects_target_and_slippage_from_three_prices(monkeypatch):
    ...
    monkeypatch.setattr("market.binance_market.service.evaluate_cow_route", fake_evaluate)
    monkeypatch.setattr("market.binance_market.service.rank_cow_routes", lambda items: items)
    monkeypatch.setattr("market.binance_market.service._cow_order_submission_adapter_available", lambda: False)
...
def test_cow_execution_attempt_history_can_fallback_to_jsonl(tmp_path):
        ...
        "ranking": [
            {
                ...
                "execution_precheck": {
                    ...
                    "reasons": ["order_submission_adapter_unavailable"],
                },
            }
        ],
    }
    ...
    assert rows[0]["blocked_reasons"] == ["order_submission_adapter_unavailable"]
...
def test_cow_network_options_exposes_supported_networks():
    from market.binance_market.service import cow_network_options
...
def test_cow_supported_token_cache_refreshes_file_and_loads_registry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "market.binance_market.service.load_cow_token_list",
        lambda *args, **kwargs: [
...
def test_cow_quote_verification_marks_requested_owner_source(monkeypatch):
    ...
    monkeypatch.setattr(
        "market.binance_market.service.build_token_registry",
